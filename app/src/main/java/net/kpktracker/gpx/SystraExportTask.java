package net.kpktracker.gpx;

import static net.kpktracker.db.DataHelper.EXTENSION_GPX;
import static net.kpktracker.util.FileSystemUtils.getUniqueChildNameFor;
...
import net.kpktracker.exception.ExportTrackException;
...
public class SystraExportTask implements Callable<String> {
    private static final String TAG = SystraExportTask.class.getSimpleName();
    ...
    private static final String TAG_GPX = "<gpx"
            ...
            + " version=\"1.1\""
            + " creator=\"KPKTracker for Android™ - https://github.com/labexp/osmtracker-android\""
    ...
    public SystraExportTask(Context context, long trackId) {
        super();
        trackID = trackId;
        this.context = context;
        pointDateFormatter.setTimeZone(TimeZone.getTimeZone("UTC"));
        try {
            tmpFile = File.createTempFile("osm-upload", ".gpx", context.getExternalCacheDir());
    ...
    protected void exportTrackAsGpx(long trackId) throws ExportTrackException {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            hasPerm = (ContextCompat.checkSelfPermission(context,
        ...
        if (hasPerm) {
            if (sdRoot.canWrite()) {
                ...
                String track_surveyor = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_SURVEYOR));
                String track_route_name = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_ROUTE_NAME));
                String track_route_id = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_ROUTE_ID));
                String track_headway_mns = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_HEADWAY_MINUTES));
                String track_route_type = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_ROUTE_TYPE));
                String track_fare_price = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_FARE_PRICE));
                String track_project = c.getString(c.getColumnIndex(TrackContentProvider.Schema.COL_PROJECT));
                ...
                if (null != cTrackPoints && null != cWayPoints) {
                    try {
                        writeGpxFile(track_name, track_surveyor, track_project, track_route_name, track_route_id,
                                track_headway_mns, track_route_type, track_fare_price, tags, track_description,
                                cTrackPoints, cWayPoints, trackFile);
                    } catch (IOException ioe) {
     ...
     * @throws IOException
     */
    private void writeGpxFile(String trackName, String track_surveyor, String track_project, String track_route_name, String track_route_id,
                              String track_headway_mns, String track_route_type, String track_fare_price,
                              String tags, String track_description, Cursor cTrackPoints, Cursor cWayPoints, File target) throws IOException {
        ...
        try {
            ...
            if ((track_project != null && !track_project.equals(""))) {
                writer.write("\t\t<project>"+ track_project +"</project>\n");
            }
            if ((track_surveyor != null && !track_surveyor.equals(""))) {
                writer.write("\t\t<surveyor>"+ track_surveyor +"</surveyor>\n");
            }
            if ((track_route_name != null && !track_route_name.equals(""))) {
                writer.write("\t\t<route_name>"+ track_route_name +"</route_name>\n");
            }
            if ((track_route_id != null && !track_route_id.equals(""))) {
                writer.write("\t\t<route_id>"+ track_route_id +"</route_id>\n");
            }
            if ((track_headway_mns != null && !track_headway_mns.equals(""))) {
                writer.write("\t\t<headway_in_minutes>"+ track_headway_mns +"</headway_in_minutes>\n");
            }
            if ((track_route_type != null && !track_route_type.equals(""))) {
                writer.write("\t\t<route_type>"+ track_route_type +"</route_type>\n");
            }
            if ((track_fare_price != null && !track_fare_price.equals(""))) {
                writer.write("\t\t<fare_price>"+ track_fare_price +"</fare_price>\n");
            }
            writer.write("\t</extensions>\n");
