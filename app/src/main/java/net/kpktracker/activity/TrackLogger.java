...
import net.kpktracker.listener.SensorListener;
import net.kpktracker.listener.PressureListener;
import net.kpktracker.receiver.MediaButtonReceiver;
import net.kpktracker.service.gps.GPSLogger;
import net.kpktracker.service.gps.GPSLoggerServiceConnection;
import net.kpktracker.util.CustomLayoutsUtils;
import net.kpktracker.util.FileSystemUtils;
import net.kpktracker.util.ThemeValidator;
import net.kpktracker.view.SystraNoteDialog;
import net.kpktracker.view.TextNoteDialog;
import net.kpktracker.view.VoiceRecDialog;
...
public class TrackLogger extends Activity {
	...
	/**
	 * constant for systra note dialog
	 */
	public static final int SYSTRA_TEXT_NOTE = 3;

	private String systraKey = "";
	 ...
	 * SYSTRA specific
	 */
	private int nbPassengers = 0;
	private int nbBoardingWomen = 0;
	private int nbBoardingMen = 0;
	private int nbAlightings = 0;
	Map<String, String> mapSystraKeyValues = new HashMap<>();
	Button systraNoteBtn;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		nbPassengers = getIntent().getExtras().getInt(TrackContentProvider.Schema.COL_NB_PASSENGERS);
		mapSystraKeyValues = new HashMap<>();
	 ...
	 * Also, the default layout is excluded and the 'kpktracker' tag is added by default.
	 */
	private void saveTagsForTrack(){
		...
		// Check if the kpktracker tag has already been added
		String trackerTag = "kpktracker";
	...
	protected void onResume() {
		...
		// Enable plus and minus buttons if nb_passengers != 0
		if ((nbPassengers != 0) && (mainLayout != null)) {
			mainLayout.setPlusAndMinusButtonsEnabled(true);
		}
	...
	public void setSystraKey(String key) {
		systraKey = key;
	}
	public String getSystraKey() {
		return systraKey;
	}
	public void setNbPassengers(int value) {
		nbPassengers = value;
	}
	...
	public int getCount(String type) {
		if (type.equals("number_passengers")) {
			return getNbPassengers();
		} else if (type.equals("boardings_men")) {
			return nbBoardingMen;
		}
		else if (type.equals("boardings_women")) {
			return nbBoardingWomen;
		}
		else if (type.equals("alightings")) {
			return nbAlightings;
		}
		return 0;
	}
	public void setCount(String type, int value) {
		if (type.equals("boardings_men")) {
			nbBoardingMen = value;
		}
		else if (type.equals("boardings_women")) {
			nbBoardingWomen = value;
		}
		else if (type.equals("alightings")) {
			nbAlightings = value;
		}
	}
	public void setSystraKeyValue(String key, String value) {
		mapSystraKeyValues.put(key, value);
	}
	public String getSystraKeyValue(String key) { return mapSystraKeyValues.get(key); }
	public void clearSystraValues() {
		nbBoardingMen = 0;
		nbBoardingWomen = 0;
		nbAlightings = 0;
		mapSystraKeyValues.clear();
	}
	public void setSystraNoteBtn(Button btn) { systraNoteBtn = btn; }
	public Button getSystraNoteBtn() { return systraNoteBtn; }
	public void clearSystraNoteBtn() { systraNoteBtn = null; }
	public void systraValidatePage(String systraType) {
		Intent intent = new Intent(OSMTracker.INTENT_TRACK_WP);
		intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
		intent.putExtra(OSMTracker.INTENT_KEY_NAME, systraType);
		sendBroadcast(intent);
		if (nbBoardingMen > 0) {
			intent = new Intent(OSMTracker.INTENT_TRACK_WP);
			intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
			intent.putExtra(OSMTracker.INTENT_KEY_NAME, "boardings_men=" + nbBoardingMen);
			sendBroadcast(intent);
		}
		if (nbBoardingWomen > 0) {
			intent = new Intent(OSMTracker.INTENT_TRACK_WP);
			intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
			intent.putExtra(OSMTracker.INTENT_KEY_NAME, "boardings_women=" + nbBoardingWomen);
			sendBroadcast(intent);
		}
		if (nbAlightings > 0) {
			intent = new Intent(OSMTracker.INTENT_TRACK_WP);
			intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
			intent.putExtra(OSMTracker.INTENT_KEY_NAME, "alightings=" + nbAlightings);
			sendBroadcast(intent);
		}
		Iterator it = mapSystraKeyValues.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry pair = (Map.Entry) it.next();
			intent = new Intent(OSMTracker.INTENT_TRACK_WP);
			intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
			intent.putExtra(OSMTracker.INTENT_KEY_NAME, pair.getKey() + "=" + pair.getValue());
			sendBroadcast(intent);
		}
		nbPassengers = nbPassengers + nbBoardingMen + nbBoardingWomen - nbAlightings;
		intent = new Intent(OSMTracker.INTENT_TRACK_WP);
		intent.putExtra(TrackContentProvider.Schema.COL_TRACK_ID, currentTrackId);
		intent.putExtra(OSMTracker.INTENT_KEY_NAME, "number_passengers=" + nbPassengers);
		sendBroadcast(intent);
		clearSystraValues();
		if (mainLayout != null && mainLayout.getStackSize() > 1) {
			mainLayout.popValidate(true);
		}
	}
	...
	protected Dialog onCreateDialog(int id) {
		switch (id) {
			case SYSTRA_TEXT_NOTE:
				// create a new SystraNoteDialog
				return new SystraNoteDialog(this, currentTrackId);
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		switch (id) {
			...
			case SYSTRA_TEXT_NOTE:
				// we need to reset Values like uuid of the dialog,
				// otherwise we would overwrite an existing waypoint
				((SystraNoteDialog) dialog).resetValues();
				break;
		}
