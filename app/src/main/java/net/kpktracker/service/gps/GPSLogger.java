				...
				if (extras != null) {
					...
					if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
						...
						if (lastLocation != null) {
							...
							// Update nb_passengers in database if name starts with "nb_passengers="
							if (name.startsWith("number_passengers=")) {
								String[] parts = name.split("=");
								int nb_passengers = Integer.parseInt(parts[1]);
								dataHelper.updateNbPassengers(currentTrackId, nb_passengers);
							}
