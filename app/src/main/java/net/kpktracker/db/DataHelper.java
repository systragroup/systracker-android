...
import net.kpktracker.db.model.TrackPoint;
import net.kpktracker.db.model.WayPoint;
...
public class DataHelper {
	 ...
	 */
	public static final String FILE_PROVIDER_AUTHORITY = "net.kpktracker.fileprovider";
	...
	public void updateNbPassengers(long trackId, int nbPassengers) {
		Uri trackUri = ContentUris.withAppendedId(TrackContentProvider.CONTENT_URI_TRACK, trackId);
		ContentValues values = new ContentValues();
		values.put(TrackContentProvider.Schema.COL_NB_PASSENGERS, nbPassengers);
	...
	public static int getNbPassengers(ContentResolver cr, long trackId) {
		int nb_passengers = 0;
		Uri trackUri = ContentUris.withAppendedId(TrackContentProvider.CONTENT_URI_TRACK, trackId);
		Cursor cursor = cr.query(trackUri, null, null,
				null, null);
		if (cursor != null && cursor.moveToFirst()) {
			nb_passengers = cursor.getInt(cursor.getColumnIndex(TrackContentProvider.Schema.COL_NB_PASSENGERS));
			cursor.close();
		}
	...
	public static File getTrackDirectory(long trackId) {
		...
		+ "/kpktracker/data/files/track" + trackId;
